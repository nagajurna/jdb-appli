app.controller("mainController", ['$scope','$http','$location','$route','authService',
	function mainController($scope, $http, $location, $route, authService) {
		...
		$scope.userRefresh = function(user) {
			$scope.loggedIn = user.loggedIn;
			$scope.currentUser = user.user;
			$scope.admin = ($scope.loggedIn && $scope.currentUser.role==="ADMIN") ? true : false;
		}
